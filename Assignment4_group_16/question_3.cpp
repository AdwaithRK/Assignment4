...
bool isRead(int eventId) // check if any reads active in table for eventId
{
    ...
    for (int i = 0; i < MAX; i++)
    {
        if (table[i].eventNo == eventId && table[i].queryType == 1)
            return true;
    }
    return false;
}
bool isWrite(int eventId) // any actrive writes for eventId there or not
{
    ...
    for (int i = 0; i < MAX; i++)
    {
        if (table[i].eventNo == eventId && (table[i].queryType == 2 || table[i].queryType == 3))
            return true;
    }
    return false;
}
...
void eventHandler(Event *event, int workerId, map<int, vector<int>> &privList)
{
    ...
    case 1: // Inquire
        // read query
        pthread_mutex_lock(&tableMutex);
        while (isWrite(event->id))
            ; // waits for conflicting write to be done
