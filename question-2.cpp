...
void parentProcess(student st[], int num_of_students, vector<int> correct_ans){
    ...
    for (int i = 0; i < num_of_students; i++)
    {
        ...
        if(status >= 0){
            cout << "Received " << status << " bytes of data in process " << getpid() << endl;
            ...
            cout << "Student - " << st[i].id << "\t Marks - " << t << endl;
        }
        else{
...
void studentProcess(key_t msg_queue_id, student *st){
    ...
    cout << "Student " << st->id << "\tans str -" << ans_str << endl;
    
    strcpy(send_buffer.text, ans_str.c_str());

    cout << "Student " << st->id << "\tans str -" << send_buffer.text << endl;
    send_buffer.mtype = 2;
...
int main(){
    ...
    for (int i = 0; i < num_of_students; i++)
    {
        ...
        if(st[i].msg_queue_id == -1){
            perror("Failure in question queue creation");
            exit(0);
        }
